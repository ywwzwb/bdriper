  ...
  <div>
    ...
    <div v-else class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      <div class="bg-card border border-border rounded-lg p-4">
        ...
        <div class="text-muted text-xs mt-1">/ {{ data?.total_tasks ?? 0 }} total</div>
      </div>

      <div class="bg-card border border-border rounded-lg p-4">
        <div class="text-muted text-sm">Goroutines</div>
      ...
      <div v-if="data?.gpu_name" class="bg-card border border-border rounded-lg p-4">
        ...
        <div class="text-muted text-xs mt-1">Usage: {{ data.gpu_percent ?? 0 }}%</div>
        <div class="w-full bg-muted rounded-full h-2 mt-2">
