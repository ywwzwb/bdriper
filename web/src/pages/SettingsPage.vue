<template>
  <div class="max-w-2xl">
    ...
    <div v-if="saved" class="text-accent mb-4">Settings saved</div>

    <div class="space-y-6">
      <div class="bg-card border border-border rounded-lg p-4">
        <label class="block text-sm text-muted mb-1">Max Concurrent Tasks</label>
      ...
      <div class="bg-card border border-border rounded-lg p-4">
        <label class="block text-sm text-muted mb-1">Preview TTL</label>
      ...
      <div class="bg-card border border-border rounded-lg p-4">
        <div class="flex items-center justify-between mb-3">
          <label class="text-sm text-muted">GPU Encoding</label>
          <div class="flex items-center gap-3">
            ...
            <button @click="testGpu" class="px-3 py-1 text-xs border border-border rounded hover:bg-muted transition" :disabled="testingGpu">Test GPU</button>
          </div>
        </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div class="bg-card border border-border rounded-lg p-4">
          <label class="block text-sm text-muted mb-1">Log Max Files</label>
        ...
        <div class="bg-card border border-border rounded-lg p-4">
          <label class="block text-sm text-muted mb-1">Log Max Size (MB)</label>
      ...
      <button @click="save" class="px-6 py-2.5 bg-accent text-black font-medium rounded-lg hover:brightness-110 transition" :disabled="saving">{{ saving ? 'Saving...' : 'Save Settings' }}</button>
    </div>

    <div class="mt-10">
      <h2 class="text-xl font-semibold mb-4">Saved Configs</h2>
      ...
      <div class="space-y-2">
        <div v-for="c in configs" :key="c.id" class="bg-card border border-border rounded-lg p-3 flex items-center justify-between">
          <div>
            ...
            <div class="text-xs text-muted">{{ c.encoder }} / {{ c.mode || 'simple' }}</div>
          </div>
          <div class="flex gap-2">
            <button @click="viewConfig(c)" class="px-2 py-1 text-xs border border-border rounded hover:bg-muted transition">View</button>
            <button @click="deleteConfig(c.id)" class="px-2 py-1 text-xs border border-destructive text-destructive rounded hover:bg-destructive/10 transition">Delete</button>
          </div>
        </div>
      </div>
    </div>
...
async function testGpu() {
  ...
  try {
    ...
    gpuError.value = e.message || 'GPU test failed'
  } finally {
